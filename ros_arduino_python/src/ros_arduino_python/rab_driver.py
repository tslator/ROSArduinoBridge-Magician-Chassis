...
class RABDriver(ArduinoBase):
    ...
    def __init__(self, params, *args, **kwargs):
        super(RABDriver, self).__init__(*args, **kwargs)
        
        self.encoder_count = 0
        self.interCharTimeout = self.timeout / 30.
    ...
    def connect(self):
        try:
            ...
            print "Configuring RAB controller ..."
            if self.set_pid_rate() == "OK":
    ...
    def set_pid_rate(self):
        print "Setting PID rate"
        cmd = 'i ' + str(self.PID_RATE)
    ...
    def pan_tilt_write(self, pan, tilt):
        pan_pos = min(SERVO_MAX, max(SERVO_MIN, degrees(pan)))
        tilt_pos = min(SERVO_MAX, max(SERVO_MIN, degrees(tilt)))
        return self.execute_ack('p %d %d' % (pan_pos, tilt_pos))
...
if __name__ == "__main__":
    ...
    myArduino = RABDriver(port=portName, baudrate=baudRate, timeout=0.5, pid_rate=10)
    myArduino.connect()
