...
class PyMataCommandHandler(threading.Thread):
    ...
    RAB_DATA = 0x73     # RAB command
    RAB_VERSION = 0x00  # RAB sub-command to return the version
    RAB_ENCODER = 0x01 # RAB sub-command to read the encoder count
    ...
    # the rab library version number.
    rab_encoder_count = (0, 0)
    rab_encoder_read_complete = False
    rab_library_version = 0
    ...
    def rab_data(self, data):
        """
        This method handles an RAB library min version message and the encoder count message
        sent from the Arduino
        @param: data - two 7 bit bytes that contain the library version number
        """
        
        # The first byte contains either the sub-command either version or encoder count
        if data[0] == self.RAB_VERSION:
            self.rab_library_version = (data[1] & 0x7f) + (data[2] << 7)
            
        elif data[0] == self.RAB_ENCODER:
            left_count = (data[1] & 0x7F) + (data[2] << 7)
            left_count += (data[3] & 0x7F) + (data[4] << 7) << 8
            left_count += (data[5] & 0x7F) + (data[6] << 7) << 16
            left_count += (data[7] & 0x7F) + (data[8] << 7) << 24
            left_count -= 2147483648
        
            right_count = (data[9] & 0x7F) + (data[10] << 7)
            right_count += (data[11] & 0x7F) + (data[12] << 7) << 8
            right_count += (data[13] & 0x7F) + (data[14] << 7) << 16
            right_count += (data[15] & 0x7F) + (data[16] << 7) << 24
            right_count -= 2147483648
            
            print left_count, right_count
            self.rab_encoder_count = (left_count, right_count)
            
            self.rab_encoder_read_complete = True
    ...
    def run(self):
        ...
        self.command_dispatch.update({self.RAB_DATA: [self.rab_data, 2]})
