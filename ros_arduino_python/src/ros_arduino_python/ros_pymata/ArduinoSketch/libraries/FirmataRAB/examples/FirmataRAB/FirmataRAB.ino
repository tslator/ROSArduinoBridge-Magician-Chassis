...
#define RAB_CONFIGURE 0
#define RAB_DRIVE 1
#define RAB_ENCODER_RESET 2
#define RAB_ENCODER_READ 3
#define RAB_PID_UPDATE 4
#define RAB_LIBRARY_VERSION 5
/* Sub-commands for returning RAB_DATA */
#define RAB_VERSION 0x00
#define RAB_ENCODER 0x01
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case RAB_DATA:
      // determine which RAB message this is
      if (argv[0] == RAB_CONFIGURE)
      {
        uint8_t pid_rate = argv[1] + (argv[2] << 7);
        if ( argc == 3 )
        {
          rab = new RAB(pid_rate);
        }
      }
      else if ( argv[0] == RAB_DRIVE )
      {
          if ( argc == 5 && rab != NULL )
          {
              double m1_speed = (double)((int)((argv[1] + (argv[2] << 7)-128))/10);
              double m2_speed = (double)((int)((argv[3] + (argv[4] << 7)-128))/10);
              rab->Drive( m1_speed, m2_speed);
              //rab->Drive( 6.0, 6.0);
          }
      }
      else if ( argv[0] == RAB_ENCODER_RESET)
      {
          if (argc == 1 && rab != NULL)
          {
      ...
      else if ( argv[0] == RAB_ENCODER_READ)
      {
          if ( argc == 1 && rab != NULL)
          {
              rab->left_encoder_count += 2147483648;
              rab->right_encoder_count += 2147483648;
            
              Firmata.write(START_SYSEX);
              Firmata.write(RAB_DATA);
              Firmata.write(RAB_ENCODER);
              Firmata.write( (rab->left_encoder_count & 0xff) & 0x7f);
              Firmata.write( (rab->left_encoder_count & 0xff) >> 7);
              rab->left_encoder_count >>= 8;
              Firmata.write( (rab->left_encoder_count & 0xff) & 0x7f);
              Firmata.write( (rab->left_encoder_count & 0xff) >> 7);
              rab->left_encoder_count >>= 8;
              Firmata.write( (rab->left_encoder_count & 0xff) & 0x7f);
              ...
              Firmata.write( (rab->right_encoder_count & 0xff) & 0x7f);
              Firmata.write( (rab->right_encoder_count & 0xff) >> 7);
              rab->right_encoder_count >>= 8;
              Firmata.write( (rab->right_encoder_count & 0xff) & 0x7f);
              Firmata.write( (rab->right_encoder_count & 0xff) >> 7);
              rab->right_encoder_count >>= 8;
              Firmata.write( (rab->right_encoder_count & 0xff) & 0x7f);
      ...
      else if ( argv[0] == RAB_LIBRARY_VERSION )
      {
        if ( argc == 1 && rab != NULL)
        {
              int version = rab->Version();
              Firmata.write(START_SYSEX);
              Firmata.write(RAB_DATA);
              Firmata.write(RAB_VERSION);
              Firmata.write(version & 0x7F);
              Firmata.write(version >> 7);
              Firmata.write(END_SYSEX);
        }
      ...
      {
        Firmata.sendString("RAB CONFIG Error: UNKNOWN RAB COMMAND");
      }
      break ;
  }
}
